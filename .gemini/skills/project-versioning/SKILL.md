...
7. **Deployment**:
   ...
   - If the user confirms, provide these manual steps:
     1. SSH to the production server: `ssh production-server`
...
- **Test**: `cargo test`
- **Git Push**: `git push origin main && git push origin v<version>`
- **SSH to production-server**: `ssh production-server`
